...
namespace nodelet_image_trim{
	...
	inline void ImageConverter::InfoCallback(const sensor_msgs::CameraInfo::ConstPtr &info){
		track2.setICP(info);
		camera_info = *info;
	}
	...
	inline void ImageConverter::imgconvCallback(const sensor_msgs::ImageConstPtr& msg  , const sensor_msgs::CameraInfo::ConstPtr &info){
		InfoCallback(info);
		try {
		...
		if(detect_flag){
			...
			if(drawpoint_flag){ 
				cv::circle(image_conved,track2.getP1(), 5, cv::Scalar(0,0,200), 1);
				cv::circle(image_conved,track2.getP2(), 5, cv::Scalar(0,0,200), 1);
				cv::circle(image_conved,track2.getP3(), 5, cv::Scalar(0,0,200), 1);
				cv::circle(image_conved,track2.getP4(), 5, cv::Scalar(0,0,200), 1);
			...
			std::cout << "detect false :" << detect_count << std::endl;
			detect_count++;
	...
	inline void ImageConverter::paste(cv::Mat dst, cv::Mat src, int x, int y) {
	...
	inline void ImageConverter::TagDetectCallback(const apriltag_ros::AprilTagDetectionArray::ConstPtr &msg){
